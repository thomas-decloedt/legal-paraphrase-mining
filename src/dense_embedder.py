...
class DenseEmbedder:
    ...
    def model(self) -> SentenceTransformer:
        if self._model is None:
            ...
            log.info(f"Loading dense embedding model: {self.config.model_name}")
            ...
            if native_dim and self.config.dimension < native_dim:
                self._model.truncate_dim = self.config.dimension
                log.info(
                    f"Truncating embeddings from {native_dim} to {self.config.dimension}"
                )
